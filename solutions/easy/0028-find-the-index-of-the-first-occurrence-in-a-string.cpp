...
class Solution {
 public:
  ...
  int strStr(std::string haystack, std::string needle) {
    ...
    // then it's impossible for it to have an occurence since the word is
    // longer.
    if (needleLength > haystackLength) {
      return -1;
    }

    // Iterate over the haystack and attempt to find the first occurence of
    // needle in haystack then return the index.
    for (int startIndex = 0; startIndex <= haystackLength - needleLength;
         startIndex++) {
...
void check(const std::string& haystack, const std::string& needle,
           int expected) {
  Solution sol;
  int got = sol.strStr(haystack, needle);

  std::cout << (got == expected ? "PASS" : "FAIL") << "  haystack=\""
            << haystack << "\""
            << "  needle=\"" << needle << "\""
