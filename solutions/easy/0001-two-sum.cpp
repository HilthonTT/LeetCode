 ...
 public:
  std::vector<int> twoSum(std::vector<int>& nums, int target) {
    ...
    for (size_t i = 0; i < nums.size(); ++i) {
      ...
      std::cout << "    need = " << target << " - " << nums[i] << " = " << need
                << "\n";

      auto it = seen.find(need);

      if (it != seen.end()) {
        std::cout << "    found " << need << " in map at index " << it->second
                  << "  -> returning {" << it->second << ", " << i << "}\n";
      ...
      std::cout << "    stored nums[i]=" << nums[i] << " -> index " << i
                << "\n";
...
void check(std::vector<int> nums, int target, std::vector<int> expected) {
  ...
  std::cout << (got == expected ? "PASS" : "FAIL")
            << "  nums=" << toString(nums) << " target=" << target
