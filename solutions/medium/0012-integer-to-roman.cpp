...
public:
    ...
    {
        std::vector<int> bases = {1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1};
...
// The same numerals, built from the digits rather than by subtracting bases: one
// table per decimal place, indexed by the digit sitting there. It never loops and
// never touches the input, so it arrives at the answer a different way from the
// solution above — which is what makes it worth comparing against.
...
// subtracted, everything else adds. Enough to confirm a numeral names the number
// it was built from, though not that it is the numeral a Roman would have used —
// "IIII" and "IV" both read as 4, which is why the shape is checked separately.
...
// The rules the problem states about how a numeral may be spelled: only the seven
// symbols, I, X, C and M repeated at most three times, and V, L and D not repeated
// at all. A numeral that reads back to the right number can still break these —
// "VIIII" is 9 — so both checks are needed to pin the answer down to one string.
...
// The constraints only allow 1 to 3999, which is few enough to check in full: the
// three properties below together leave exactly one valid numeral per number. One
// line per value would bury the cases above in sixteen thousand lines of output,
// so the sweep reports each failure in the usual detail and everything else as a
// single line.
void sweep(int low, int high)
{
    ...
    for (int num = low; num <= high; num++)
    {
        ...
        std::cout << "FAIL"
                  ...
                  << "\n  got       " << got
                  << "\n  reads as  " << readBack
                  << "\n  spelling  " << (wellFormed ? "ok" : "breaks the repetition rules")
                  << "\n"
                  << std::string(60, '-') << "\n";
    }

    std::cout << (failures == 0 ? "PASS" : "FAIL")
              << "  num=" << low << ".." << high << " (every value in range)"
              << "\n  expected " << (high - low + 1) << " numerals matching the reference, reading back, well formed"
