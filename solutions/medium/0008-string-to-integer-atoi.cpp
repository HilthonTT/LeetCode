...
// The same parse, but accumulating into a type wide enough to hold the value and
// clamping only at the end. Slower and reliant on 64-bit arithmetic the real
// solution avoids, which is what makes it useful for the generated cases below:
// it gets to the answer a different way.
...
// For the generated inputs, whose answers are not worth spelling out one by one.
void checkAgainstReference(const std::string &s)
{
    check(s, reference(s));
}
...
int main()
{
    ...
    for (long long magnitude = 2147483640LL; magnitude <= 2147483655LL; magnitude++)
    {
    ...
    const std::vector<std::string> digits = {"0", "7", "42", "2147483647", "2147483648", "91283472332"};
