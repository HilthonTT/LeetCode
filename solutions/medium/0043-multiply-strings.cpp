...
class Solution {
 public:
  ...
  std::string multiply(std::string num1, std::string num2) {
    ...
    // Initialize result array with size len1 + len2 (maximum possible digits in
    // product)
...
void check(const std::string& num1, const std::string& num2,
...
void checkBothWays(const std::string& num1, const std::string& num2,
                   const std::string& expected,
                   const std::string& description = "") {
  check(num1, num2, expected, description);
  check(num2, num1, expected,
        description.empty() ? "" : description + ", arguments swapped");
}

// The string of n nines, which is 10^n - 1.
std::string nines(int n) { return std::string(n, '9'); }

// A 1 followed by n zeros, which is 10^n.
std::string powerOfTen(int n) { return "1" + std::string(n, '0'); }
...
int main() {
  ...
  check("9", "9", "81",
        "the largest single-digit product, which carries into two digits");

  // --- products one digit shorter than len1 + len2, so a leading zero is
  // skipped ---
  check("11", "11", "121");
  check("12", "12", "144");
  ...
  check("99", "99", "9801");
  check("32", "32", "1024",
        "the first two-by-two product that needs all four digits");
  ...
  checkBothWays("9", nines(50), "8" + nines(49) + "1",
                "one digit against fifty nines");
  ...
  checkBothWays("1020", "3040", "3100800",
                "interior and trailing zeros in both operands");

  // --- products past what a 64-bit integer could hold ---
  check("123456789", "987654321", "121932631112635269",
        "the largest of these that still fits in int64");
  check("999999999999999999", "999999999999999999",
        "999999999999999998000000000000000001",
        "thirty-six digits, past the int64 ceiling");
  check("12345678901234567890", "98765432109876543210",
        "1219326311370217952237463801111263526900", "forty digits");
