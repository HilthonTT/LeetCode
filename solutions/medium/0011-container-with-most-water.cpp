...
// Every pair of lines, measured directly. Quadratic, so it is useless on the sizes
// the problem actually allows, but it makes no argument about which pairs can be
// skipped — and skipping the right pairs is the entire claim the two-pointer walk
// above is making. That makes it the thing worth comparing against.
long long reference(const std::vector<int> &height)
{
    ...
    for (size_t i = 0; i < height.size(); i++)
    {
        ...
        {
            long long water = static_cast<long long>(std::min(height[i], height[j])) * static_cast<long long>(j - i);
...
// A pass through every height array of the given length whose values are 0..maxHeight,
// counted like an odometer. Small ranges only — the count is maxHeight+1 to the power
// of the length — but exhaustive at those sizes, which is where an off-by-one in the
// pointer walk or a mishandled tie between the two ends would show up. Printing one
// line per array would drown the cases above, so only failures are reported in full.
void sweep(int length, int maxHeight)
{
    ...
    std::cout << (failures == 0 ? "PASS" : "FAIL")
              << "  height=every array of length " << length << " with values 0.." << maxHeight
              << "\n  expected " << cases << " arrays matching the all-pairs reference"
              << "\n  got      " << (cases - failures) << " of them"
              << "\n"
              << std::string(60, '-') << "\n";
}

// The exhaustive sweep can only afford short arrays, and short arrays never put the
// two pointers far enough apart for the skipping to matter much. These are longer and
// still checked against the same reference; the generator is a fixed linear congruence
// so a failure here names an array that can be reproduced by running the file again.
void randomSweep(int arrays, int length, int maxHeight)
{
    ...
    std::cout << (failures == 0 ? "PASS" : "FAIL")
              << "  height=" << arrays << " pseudo-random arrays of length " << length
              << ", values 0.." << maxHeight
              << "\n  expected " << arrays << " matching the all-pairs reference"
...
int main()
{
    ...
    // --- heights that only rise, or only fall: the best pair is the outermost one ---
    check({1, 2, 3, 4, 5}, 6);
    check({5, 4, 3, 2, 1}, 6);
    ...
    // --- large values, where the area overflows a 16-bit intermediate but not an int ---
