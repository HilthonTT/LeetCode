...
std::string lettersFor(char digit) {
  static const std::vector<std::string> keypad = {"abc", "def",  "ghi", "jkl",
                                                  "mno", "pqrs", "tuv", "wxyz"};
...
void check(const std::string& digits, std::vector<std::string> expected) {
  ...
  std::cout << (gotSorted == expected ? "PASS" : "FAIL") << "  digits=\""
            << digits << "\""
...
void checkShape(const std::string& digits, const std::string& description) {
  ...
  if (got.size() != expectedCount) {
    problem = "expected " + std::to_string(expectedCount) +
              " combinations, got " + std::to_string(got.size());
  }
  ...
  for (const std::string& combination : got) {
    ...
    if (combination.size() != digits.size()) {
      problem = "\"" + combination + "\" is not " +
                std::to_string(digits.size()) + " letters long";
      break;
    }
    ...
    for (std::size_t i = 0; i < digits.size(); ++i) {
      ...
      if (letters.find(combination[i]) == std::string::npos) {
        problem = "\"" + combination + "\" has '" + combination[i] +
                  "' where digit " + digits[i] + " only allows \"" + letters +
                  "\"";
        break;
      }
    }
  ...
  std::cout << (problem.empty() ? "PASS" : "FAIL") << "  digits=\"" << digits
            << "\""
...
int main() {
  ...
  check("77", {"pp", "pq", "pr", "ps", "qp", "qq", "qr", "qs", "rp", "rq", "rr",
               "rs", "sp", "sq", "sr", "ss"});

  // --- the two four-letter keys, alone and together ---
  check("79", {"pw", "px", "py", "pz", "qw", "qx", "qy", "qz", "rw", "rx", "ry",
               "rz", "sw", "sx", "sy", "sz"});

  // --- keys of different sizes, in both orders, so a letter cannot drift
  //     from one position to the other ---
  check("29", {"aw", "ax", "ay", "az", "bw", "bx", "by", "bz", "cw", "cx", "cy",
               "cz"});
  check("92", {"wa", "wb", "wc", "xa", "xb", "xc", "ya", "yb", "yc", "za", "zb",
               "zc"});
